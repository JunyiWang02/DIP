...
def create_mask_from_points(points, img_h, img_w):
    ...
    # 使用 cv2.fillPoly 函数填充多边形区域
    # 将 points 转换为适合 fillPoly 的形状
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    # print (foreground_img.shape,foreground_mask.shape,background_mask.shape)
    ### Note: The loss is computed within the masks.
    laplacian_tensor= torch.tensor([[0., 1., 0.], 
                        [1., -4., 1.], 
                        [0., 1., 0.]], 
                        dtype=foreground_img.dtype, 
                        device=foreground_img.device)
    laplacian_tensor_expanded = laplacian_tensor.unsqueeze(0).unsqueeze(0)  
    laplacian_tensor_final = laplacian_tensor_expanded.repeat(1, 3, 1, 1)   
    laplacian_foreground = F.conv2d(foreground_img, 
                                     laplacian_tensor_final,
                                     padding=1,groups=1)
    # print (laplacian_foreground.shape)
    laplacian_blended = F.conv2d(blended_img, 
                                  laplacian_tensor_final,
                                  padding=1,groups=1)
    # print (laplacian_blended.shape)
    # Apply the masks to compute the loss
    # masked_laplacian_foreground = laplacian_foreground * foreground_mask
    # masked_laplacian_blended = laplacian_blended * background_mask
    
    
    # values1 = masked_laplacian_foreground[foreground_mask.bool()].view(-1)
    # values2 = masked_laplacian_blended[background_mask.bool()].view(-1)
    
    # # 确保两者的像素点数量一致
    # min_len = min(values1.size(0), values2.size(0))
    # values1 = values1[:min_len]
    # values2 = values2[:min_len]
    non_zero_indices_fg = foreground_mask > 0
    non_zero_indices_bg = background_mask > 0

    # Select only the masked region and calculate MSE loss
    loss = F.mse_loss(laplacian_foreground[non_zero_indices_fg], laplacian_blended[non_zero_indices_bg])
    # size = (background_mask.shape[2],background_mask.shape[3])
    # masked_laplacian_foreground = F.interpolate()
    # Calculate the loss as the mean squared error between the masked Laplacians
    # loss = F.mse_loss(masked_laplacian_foreground, masked_laplacian_blended)
    # loss = F.mse_loss(values1, values2)
    return loss
...
def blending(foreground_image_original, background_image_original, dx, dy, polygon_state):
    ...
    # print(fg_img_tensor.shape,mask_expanded.shape,bg_mask_tensor.shape)
