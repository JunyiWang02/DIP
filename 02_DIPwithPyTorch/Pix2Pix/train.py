...
def load_weights(model, pretrained_weights_path, device):
    # Load the state_dict from the pretrained weights
    state_dict = torch.load(pretrained_weights_path, map_location=device)

    # Create a new state_dict without the 'module.' prefix if it exists
    new_state_dict = {}
...
def main():
    ...
    # pretrained_weights_path = '/home/juyonggroup/shared3dv/repos/wjy/DIP/02_DIPwithPyTorch/Pix2Pix/checkpoints/pix2pix_model_epoch_40.pth'
    
    # load_weights(model, pretrained_weights_path, device)
    model.to(device)
    criterion = nn.L1Loss()
